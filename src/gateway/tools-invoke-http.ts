...
// Strip stack traces, obfuscate filesystem paths, and bound length before
// returning tool errors over HTTP. Keeps response bodies free of internal
// stack/path detail.
...
export async function handleToolsInvokeHttpRequest(
  ...
  if (outcome.ok) {
    ...
    const error =
      outcome.error.type === "tool_error"
        ? { ...outcome.error, message: sanitizeErrorMessage(outcome.error.message) }
        : outcome.error;
    sendJson(res, outcome.status, { ok: false, error });
  }
