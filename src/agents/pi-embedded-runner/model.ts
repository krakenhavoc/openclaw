...
function resolveAzureHeaders(
  providerConfig: { apiKey?: unknown } | undefined,
...
function applyConfiguredProviderOverrides(params: {
  ...
  const azureHeaders = isAzure
    ? resolveAzureHeaders(providerConfig as Record<string, unknown>)
    : undefined;
  if (isAzure) {
  ...
  // Apply Azure-specific base URL and header transformations on top of the
  // resolved request config so Azure deployments get the correct path and
  // api-key header regardless of the provider request config policy.
  const resolvedBaseUrl =
    resolveAzureBaseUrl(requestConfig.baseUrl, modelId) ??
    requestConfig.baseUrl ??
    discoveredModel.baseUrl;
  const resolvedHeaders =
    isAzure && azureHeaders ? { ...requestConfig.headers, ...azureHeaders } : requestConfig.headers;
